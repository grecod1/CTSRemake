//This Script is used for adding a list of emails under the Create Routing Category View

var counter = 0; //Keeps track of how many emails are on the list, a maximium of 20 emails
var regex = '^[a-z|A-Z|0-9|\.|_|-]+[@]{1}[a-z|A-Z|0-9|\.|_|-]+[\.]{1}[a-zA-z]{1,5}$'; //regular expression that determines if email is valid
...
$('#addEmail').on('paste', function (event) {    
    var content;
        ...
        if (emailInput.match(regex)) {
            ...
            if (sameEmail) {
                ...
                $('#emailTable').append('<tr><td class="tdEmail w-75">' + emailInput
                    ...
                    + 'class="BTNRemove btn btn-outline-danger btn-sm">remove</button></td></tr>');
                counter++;
                $('#addEmail').val('');                
                setNameProperties();
                buttonAnimation(false);
...
function setNameProperties() {
    var index = 0;
    $('.EmailListItem').each(function (index, elememt) {
        elememt.setAttribute('name', 'Emails[' + index + ']');
        index++;
    })
    index = 0;
}
